...
- **`coordinator.py` — `TeisonCoordinator`.** One coordinator per charger. The poll interval defaults to 30s (`DEFAULT_SCAN_INTERVAL`) but is **user-overridable via the options flow** (`entry.options[CONF_SCAN_INTERVAL]`). The two upstream calls poll at **different cadences**: `deviceDetail` (telemetry) every cycle, `getCpConfig` (settings) only on the first refresh and then at most once per `CONFIG_POLL_INTERVAL` (10 min) — the last config is cached and reused in between, so `TeisonData(detail, config)` keeps its shape. A write forces an out-of-band config refresh via `async_request_config_refresh()`. After each successful update the coordinator also raises/clears a **repair issue** (`translation_key="charger_faulted"`) when `connStatus` maps to `faulted`. `TeisonAuthError` becomes `ConfigEntryAuthFailed` (kicks off HA reauth); other errors become `UpdateFailed`. The coordinator lives on `entry.runtime_data` (typed `TeisonConfigEntry`), not `hass.data`.

- **`config_flow.py`.** User step logs in, lists chargers, and either auto-creates the entry (single charger) or shows a picker. Unique ID is the charger serial (falling back to its id). Reauth re-prompts for the password only. A **snapshot of device metadata** (`DATA_DEVICE_INFO`) is captured here so `entity.py` can build the device-registry entry without an extra API round-trip. `TeisonOptionsFlow` exposes the scan interval; changing it fires an update listener (registered in `__init__.py`) that reloads the entry.

- **Entity platforms** (`sensor.py`, `binary_sensor.py`, `switch.py`, `number.py`) all extend `TeisonEntity` (`entity.py`), read straight from `coordinator.data`, and use `_attr_has_entity_name` + `translation_key`. Sensors/binary sensors/numbers are table-driven via `*EntityDescription` dataclasses with a `value_fn`/`is_on_fn` lambda reading `detail`/`config` dicts. Writes (switch on/off, number set) call the client, then request a refresh — number writes use `async_request_config_refresh()` so the new setting shows immediately.

- **Status mapping** lives in `const.py`: numeric `connStatus` → OCPP-style enum strings (`CHARGE_POINT_STATUS`). `STATUS_CHARGING = 2` is the "is charging" check; `STATUS_FAULTED = "faulted"` (connStatus 8/88) drives the repair issue and the `problem` binary sensor. Config write keys (`VendorMaxWorkCurrent`, etc.) are constants here too.

Strings/translations: `strings.json` is the source; `translations/en.json` mirrors it. Keep them in sync when adding entities, flow steps, options steps, or repair `issues`.
